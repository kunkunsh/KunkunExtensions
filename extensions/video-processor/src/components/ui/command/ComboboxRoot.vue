...
async function onOpenChange(val: boolean) {
  ...
  if (val) {
    ...
    if (props.resetSearchTermOnBlur) resetSearchTerm();
  }
}
...
const { getItems, reactiveItems, itemMapSize } = createCollection<{ value: T }>(
...
const filteredOptions = computed(() => {
  if (isUserInputted.value) {
    ...
    if (optionsWithTypeString.length)
      return optionsWithTypeString.filter((i) =>
...
const activeIndex = computed(() =>
  filteredOptions.value.findIndex((i) => isEqual(i, selectedValue.value)),
);
...
provideComboboxRootContext({
  ...
  onInputNavigation: async (val) => {
    ...
    if (
      (index === 0 && val === "up") ||
      (index === filteredOptions.value.length - 1 && val === "down")
    )
      return;
  ...
  onInputEnter: async () => {
    if (
      filteredOptions.value.length &&
      selectedValue.value &&
      selectedElement.value instanceof Element
    )
